...
class ContextManager:
    ...
    async def get_channel_context(channel: discord.TextChannel, 
                                bot_user_id: int,
                                target_message: Optional[discord.Message] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            async for message in channel.history(limit=ContextManager.MAX_CONTEXT_MESSAGES + 5):
                ...
                content = await ContextManager._process_message_content(message)
                if not content:
                    continue
    ...
    @staticmethod
    async def _process_message_content(message: discord.Message) -> str:
        """
    ...
    def build_prompt(context_messages: List[Dict[str, Any]], 
        ...
        try:
            ...
            if is_ping and not current_message.strip():
                ...
                prompt_parts.append(f"{current_user} is trying to get your attention (they pinged you but didn't say anything specific).")
